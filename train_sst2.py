...
class TextClassifier:
    """
    文本分类器类，用于SST-2（斯坦福情感树库）任务
    使用BERT模型提取特征，然后使用逻辑回归进行二分类（积极/消极情感）
    """
    ...
    def train_and_evaluate(self, train_path, val_path):
        ...
        # 计算多个评估指标
        print("\n验证集指标：")
        print("准确率:", accuracy_score(pred_pd["label"], pred_pd["prediction"]))
        print("MCC:", matthews_corrcoef(pred_pd["label"], pred_pd["prediction"]))
        print("F1:", f1_score(pred_pd["label"], pred_pd["prediction"]))
        # 输出详细的分类报告
...
def main():
    ...
    train_path = "processed_data/sst2_train.tsv"  # 训练集路径
    val_path = "processed_data/sst2_dev.tsv"  # 验证集路径
